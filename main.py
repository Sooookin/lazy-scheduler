...
def stub_ssl():
    """ssl 이 없는 빌드에서만 자리에 껍데기를 끼운다.

    예전 빌드는 용량(5.8MB)을 줄이려고 ssl 을 뺐다. 지금은 동기화가 HTTPS 를
    쓰므로 빌드에 들어 있고, 이 함수는 진짜 ssl 을 찾으면 아무것도 하지 않는다.
    ssl 이 빠진 빌드에서도 창(pywebview 가 최상단에서 import ssl 한다)은 뜨게 남겨 둔다.
